...
# The sources carry no license file, so nothing may be assumed about them.
...
# it round, which is what a license granting nothing forbids.
...
# unRAR license, which is not in @FREE, so asking for it would stop the emerge
